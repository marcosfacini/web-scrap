...
Isso significa que eu irei escrever um código que pega o URL base do site e percorre as páginas que eu bem entender, 
não focando somente em uma única página (se não entendeu não se preocupe, veremos melhor adiante)

O que vamos coletar?
Vamos coletar as Imagens de todas as habilidades de todos os campeões do League of Legends.

Mas porque vamos coletar isso?
Porque eu tenho vontade de escrever um APP em que eu vou utilizar as imagens, 
e para não ter que baixar na mão mais de 140 (campeões) * 4 (Habilidades) = 560 imagens 
e além disso ter que renomear cada uma para o padrão de nomes que eu escolher.
...
def getdata(url): 
    r = requests.get(url) 
    return r.text
Aqui estamos fazendo uma requisição da URL escolhida e imprimindo o texto de resposta, que normalmente é o HTML inteiro da página, 
um exemplo de print do próprio código é:
